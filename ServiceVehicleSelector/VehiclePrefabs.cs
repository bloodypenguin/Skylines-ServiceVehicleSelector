...
// Type: ServiceVehicleSelector.VehiclePrefabs
    ...
    for (int index = 0; index < PrefabCollection<VehicleInfo>.PrefabCount(); ++index)
      {
        ...
        if ((Object) prefab != (Object) null && !VehiclePrefabs.IsTrailer(prefab))
        {
          ...
          {
            if (prefab.m_class.m_subService == ItemClass.SubService.PublicTransportTrain && prefab.m_class.m_level == ItemClass.Level.Level4)
