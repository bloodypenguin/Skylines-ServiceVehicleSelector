    ...
    if (flag1 && flag2 && instance1.CreateVehicle(out vehicle1, ref Singleton<SimulationManager>.instance.m_randomizer, vehicleInfo, position1, (TransferManager.TransferReason) vehicleData.m_transferType, false, true))
    {
      ...
      else
      {
        ...
        if (ServiceVehicleSelectorMod.BuildingData.TryGetValue(cargoStationId, out var source) && source.Count > 0)
        {
          ...
          info2 = VehicleProvider.GetVehicleInfo(ref Singleton<SimulationManager>.instance.m_randomizer, info1.m_class.m_service, info1.m_class.m_subService, ItemClass.Level.Level4, prefabName);
        }
        else
        {
          info2 = null;
